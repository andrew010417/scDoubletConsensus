...
#' @param PCs Principal components passed to [run_doubletfinder()].
#' @param expected_doublet_rate Passed to [run_doubletfinder()].
#' @param consensus_method `"rank"` or `"weighted"`, passed to
#'   [consensus_rank()].
#' @param weights Optional named weights for `doubletfinder`/`scdblfinder`,
#'   passed to [consensus_rank()].
#' @param top_n,quantile Passed to [classify_consensus_doublets()].
...
run_consensus_pipeline <- function(seurat_obj,
  ...
  consensus_df <- classify_consensus_doublets(consensus_df, top_n = top_n, quantile = quantile)
